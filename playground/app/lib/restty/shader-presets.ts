...
export const SHADER_PRESETS: Array<{ id: ShaderPreset; label: string }> = [
  { id: "none", label: "None" },
  { id: "scanline", label: "Scanline" },
  { id: "aurora", label: "Aurora" },
  { id: "crt-lite", label: "CRT Lite" },
  { id: "mono-green", label: "Mono Green" },
];
...
fn resttyStage(color: vec4f, uv: vec2f, time: f32, params0: vec4f, params1: vec4f) -> vec4f {
  ...
  let tint = vec3f(0.12 + wave0 * 0.28, 0.08 + wave1 * 0.32, 0.18 + wave2 * 0.24);
...
vec4 resttyStage(vec4 color, vec2 uv, float time, vec4 params0, vec4 params1) {
  ...
  vec3 tint = vec3(0.12 + wave0 * 0.28, 0.08 + wave1 * 0.32, 0.18 + wave2 * 0.24);
