...
// ctx.restty is the narrowed host surface. Prefer ctx.restty.activePane()
// / ctx.restty.pane(id) for pane-local terminal actions, and use the same
// host for higher-level shell ops.
