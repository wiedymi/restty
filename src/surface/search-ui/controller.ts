...
export function createPaneSearchUiController(
  ...
  function open(paneId: number, config: ResttyPaneSearchUiOpenOptions = {}): void {
    ...
    paneState.open = true;
    paneState.state = { ...paneState.pane.runtime.search.getState() };
    syncPaneUi(paneState);
  ...
  function registerPane(pane: PaneSearchUiState["pane"]): void {
    ...
    const paneState: PaneSearchUiState = {
      ...
      status,
      cleanupFns: [],
      state: { ...pane.runtime.search.getState() },
      open: false,
    };

    const onInput = () => {
      pane.runtime.search.setQuery(input.value);
    };
    ...
    const onInputKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Enter") {
        ...
        if (event.shiftKey) {
          pane.runtime.search.next();
        } else {
          pane.runtime.search.previous();
        }
        return;
      }
      if (event.key === "Escape") {
    ...
    const onClear = () => {
      pane.runtime.search.clear();
      paneState.state = { ...pane.runtime.search.getState() };
      syncPaneUi(paneState);
      focusSearchInput(paneState);
    };
