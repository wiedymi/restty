...
const srgbChannelToLinear = (c: number) => (c <= 0.04045 ? c / 12.92 : Math.pow((c + 0.055) / 1.055, 2.4));
...
function drawKittyOverlay(placements: KittyPlacement[], cellW: number, cellH: number) {
  ...
  if (KITTY_OVERLAY_DEBUG) {
    ...
    const sample = placements
      ...
      .join(" | ");
    const byImage = [...groups.entries()]
      .map(([id, count]) => `${id}:${count}`)
      .join(",");
    const sig = `${placements.length}|${byImage}|${sample}`;
    if (sig !== kittyOverlayDebugLastSig) {
      kittyOverlayDebugLastSig = sig;
      console.log(`[kitty-overlay] count=${placements.length} cell=${cellW}x${cellH} images=${byImage} sample=${sample}`);
    }
  }
...
inputHandler = createInputHandler({
  ...
  getWindowMetrics: () => {
    ...
    const cellWidthPx = Math.max(
      1,
      Math.round(gridState.cellW || widthPx / Math.max(1, cols)),
    );
    const cellHeightPx = Math.max(
      1,
      Math.round(gridState.cellH || heightPx / Math.max(1, rows)),
    );
...
function noteColorGlyphText(entry: FontEntry, text: string, shaped: { glyphs: Array<{ glyphId: number }> }) {
...
function tickWebGPU(state) {
  ...
  if (cursorStyle !== null && cursorPos) {
    ...
    if (cursorRow < rows && cursorCol < cols) {
      ...
      if (cursorStyle === 0) {
        ...
        pushRect(cursorData, x, y + cellH - cursorThicknessPx, cursorWidth, cursorThicknessPx, cursorColor);
        pushRect(cursorData, x, y, cursorThicknessPx, cellH, cursorColor);
        pushRect(cursorData, x + cursorWidth - cursorThicknessPx, y, cursorThicknessPx, cellH, cursorColor);
      } else {
  ...
  const uploadInstanceBatches = (
    ...
    const buffer =
      kind === "rect" ? state.rectInstanceBuffer : state.glyphInstanceBuffer;
    for (const batch of batches) {
